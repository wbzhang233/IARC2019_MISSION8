...
package com.dji.P4MissionsDemo.tensorflow.tracking;
...
import com.dji.P4MissionsDemo.tensorflow.Classifier.Recognition;
...
public class MultiBoxTracker {
    ...
    private List<RectF> mPos = new LinkedList<RectF>();
    public List<RectF> mPositionGet(){
        return mPos;
    }
    ...
    public synchronized void draw(final Canvas canvas) {
        ...
        int canvWidth = (int) (multiplier * (rotated ? frameHeight : frameWidth));
        int canvHeight = (int) (multiplier * (rotated ? frameWidth : frameHeight));
        frameToCanvasMatrix =
        ...
        for (final TrackedRecognition recognition : trackedObjects) {
            ...
            trackedPos.left = trackedPos.left + (canvas.getWidth() - canvWidth)/2;
            trackedPos.right = trackedPos.right + (canvas.getWidth() - canvWidth)/2;
            trackedPos.bottom = trackedPos.bottom + (canvas.getHeight() - canvHeight)/2;
            trackedPos.top = trackedPos.top + (canvas.getHeight() - canvHeight)/2;
            mPos.add(trackedPos);
